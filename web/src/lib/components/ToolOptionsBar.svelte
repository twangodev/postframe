...
{#snippet readout(label: string, value: string)}
	<span class="shrink-0">{label} <span class="font-mono text-text">{value}</span></span>
{/snippet}

{#snippet toggle(label: string, checked: boolean = true)}
	<label class="flex shrink-0 cursor-pointer items-center gap-1.5">
		<input type="checkbox" {checked} class="size-3 accent-accent" />
		{label}
	</label>
{/snippet}
...
>
	...
	{:else if selectionTools.has(activeTool)}
		...
		{#if activeTool === 'magic-wand'}
			{@render readout('tolerance', '32')}
		{/if}
		{@render readout('feather', '0 px')}
		{@render toggle('anti-alias')}
		{#if activeTool === 'magic-wand'}
			{@render toggle('contiguous')}
		{/if}
	{:else if cropTools.has(activeTool)}
		...
		{@render toggle('delete cropped pixels', false)}
	{:else if retouchTools.has(activeTool) || paintTools.has(activeTool)}
		...
		{@render readout('hardness', '65%')}
		{@render readout('opacity', '100%')}
		{#if ['brush', 'pencil', 'mixer-brush'].includes(activeTool)}
			{@render readout('flow', '100%')}
		{:else}
			{@render toggle('sample all layers')}
		{/if}
	...
	{:else if vectorTools.has(activeTool)}
		...
		{@render readout('stroke', '1 px')}
	{:else if measureTools.has(activeTool)}
		{@render readout('sample', '5 × 5')}
		{@render readout('scale', '1 px : 1 px')}
	...
	{:else if activeTool.startsWith('mask')}
		{@render readout('size', `${tools.refineBrushSize} px`)}
		{@render readout('feather', '45%')}
		{@render readout('flow', '100%')}
		{#if activeTool === 'mask'}
			{@render readout('mode', tools.maskBrushOperation)}
		{/if}
	{:else}
		{@render toggle('auto-select')}
		{@render toggle('show transform controls', false)}
	{/if}
</div>
