...
test('edits a linear gradient with its on-canvas handles', async ({ page }) => {
	...
	const gizmoLines = viewport.locator('svg[preserveAspectRatio="none"] line');
	...
	const addRadialMaskTakingTheSelection = async () => {
		await aside.getByRole('button', { name: 'radial', exact: true }).click();
	...
	const reselectTheLinearMask = () =>
		aside.getByRole('button', { name: /linear gradient/ }).click();
	await addRadialMaskTakingTheSelection();
	await reselectTheLinearMask();
	await expect(gizmoLines).toHaveCount(6);

	const dragThePositiveEndpointDot = () =>
		dragStroke(page, pointAt(rect, 0.7, 0.5), pointAt(rect, 0.5, 0.28));
	await dragThePositiveEndpointDot();
	await expect
	...
	const dragTheDashedCenterLineThroughTheAnchor = () =>
		dragStroke(page, pointAt(rect, 0.5, 0.5), pointAt(rect, 0.58, 0.6));
	await dragTheDashedCenterLineThroughTheAnchor();
	await expect
